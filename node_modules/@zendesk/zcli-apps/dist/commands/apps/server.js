...
class Server extends core_1.Command {
    async run() {
        ...
        const appPaths = (0, shared_1.getAppPaths)(appDirectories);
        let appJSON = await (0, buildAppJSON_1.buildAppJSON)(appPaths, port);
        const app = express();
        app.use(cors());
        ...
        const watchers = appPaths.map(appPath => fs.watch(appPath, async (eventType, filename) => {
            if (filename.toLowerCase() === 'manifest.json') {
                // Regenerate app.json
                appJSON = await (0, buildAppJSON_1.buildAppJSON)(appPaths, port);
...
Server.description = 'serves apps in development mode';
Server.flags = {
    help: core_1.Flags.help({ char: 'h' }),
    bind: core_1.Flags.string({ default: 'localhost', description: 'Bind apps server to a specific host' }),
    port: core_1.Flags.string({ default: '4567', description: 'Port for the http server to use' }),
